 ...
 * 20 Questions Game, based on questions that have already been asked.
 * The QuestionTree stores the possible objects in a tree indexed by
 * their properties.  
 * 
...
public abstract class QuestionTreeBase {
	...
	/**
	 * This method updates the tree based on the response to the query.
	 * If the query is satisfied (i.e., answer is true), then the m_searchRoot is updated
	 * to the matching node in the tree. Otherwise, the matching node in the tree is eliminated
	 * and the tree is re-trimmed.
	 * 
	 * @param query the question that is asked
	 * @param answer the answer to the query (true/false)
	 */
	...
	/**
	 * This method constructs a query based on the given node in the tree.
	 * If the chosen object satisfies the query, then it is guaranteed to be in the
	 * subtree of this node. If the chosen object does not satisfy the query, then
	 * it is guaranteed not to be in the subtree indicated by this node.
	 * 
	 * @param node the node whose properties we want to turn into a query
	 ...
	 */
	public TreeNode<String> search(Query query){
	...
	/**
	 * This method counts the number of objects that are descendents of a node.
	 * 
	...
	/**
	 * This method returns any one object from the tree.
	 * This is most useful if there is only one object left in the tree.
	 * 
	...
	/**
	 * This method prints all the objects in the tree,
	 * specifying the root-to-leaf path of each object.
	 */
